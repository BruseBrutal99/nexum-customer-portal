/** ISO country codes — aligned with Nexum-TMS localization list (labels = codes for hydration safety). */
...
export const COUNTRY_OPTIONS: CountryOption[] = Array.from(
  new Set(["DK", ...COUNTRY_CODE_LIST]),
)
  .map((code) => ({ code, label: code }))
  .sort((a, b) => a.code.localeCompare(b.code, "en"));

export const COLLI_COUNT_OPTIONS = Array.from({ length: 20 }, (_, i) => i + 1);
